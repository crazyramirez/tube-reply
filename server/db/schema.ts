...
export const videoTranscripts = sqliteTable('video_transcripts', {
  ...
  // 'ok' | 'no_captions' | 'forbidden' | 'error' — skip retry when not 'ok'
  fetchStatus: text('fetch_status').default('ok'),
  fetchedAt: text('fetched_at').default(sql`(datetime('now'))`),
}, (t) => ({
  videoLangUniq: uniqueIndex('video_transcripts_video_lang_unique').on(t.videoId, t.language),
  videoStatusIdx: index('video_transcripts_status_idx').on(t.videoId, t.fetchStatus),
}))

// ─── Video Analytics (YouTube Analytics API) ─────────────────────────────────
...
export const videoAnalytics = sqliteTable('video_analytics', {
  ...
  averageViewDuration: real('average_view_duration').default(0), // seconds
  averageViewPercentage: real('average_view_percentage').default(0), // 0-100
  impressions: integer('impressions').default(0),
  impressionCtr: real('impression_ctr').default(0), // 0.0-1.0
  ...
  videoDateUniq: uniqueIndex('video_analytics_video_date_unique').on(t.videoId, t.snapshotDate),
  videoIdx: index('video_analytics_video_idx').on(t.videoId),
  dateIdx: index('video_analytics_date_idx').on(t.snapshotDate),
}))
